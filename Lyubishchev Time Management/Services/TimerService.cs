...
public sealed class TimerService(AppDbContext dbContext, IClock clock, TimeEntryService entryService, IOperationalEventLogger operationalEventLogger)
{
    ...
    public async Task<StopTimerResult> StopAsync(ulong userId, string? name, ulong? categoryId, List<string>? tagNames, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                // Guards the EndTimeUtc > StartTimeUtc check constraint if Stop is called in the
                // same clock tick as Start (fast automated calls, low-resolution system clocks, etc.).
        ...
        catch (Exception exception) when (exception is not DbUpdateConcurrencyException)
        {
            // The known DbUpdateConcurrencyException path above already returns its own business
            // result without reaching here. Anything else is genuinely unexpected: log it (never
            // at Warning/Info -- an operator needs to see this), roll back if a transaction was
            // actually started, and let the global exception handler own the HTTP 500.
            operationalEventLogger.TimerTransactionFailed(userId, "Stop", exception);
