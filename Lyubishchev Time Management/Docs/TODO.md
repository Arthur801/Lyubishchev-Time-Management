...
- [x] 全域例外處理：`Infrastructure/Errors/GlobalExceptionHandler.cs`（`IExceptionHandler`），`/api/*` 回安全 RFC 7807 Problem Details（`traceId` extension），其餘路徑安全渲染 `/Home/Error`（同一個 `traceId`），所有環境一致（不再限定 production），已知的 400/401/404/409/429 業務錯誤完全不受影響
- [x] `Infrastructure/Logging/`：`IAuthEventLogger`/`AuthEventLogger` 完成 auth failure/success 記錄；新增 `IOperationalEventLogger`/`OperationalEventLogger` 記錄 Timer transaction failure（Event ID 1001）與 CSV export failure（Event ID 1002）的安全 metadata（不含使用者輸入內容）；DB 相關的其他未預期失敗交給 `GlobalExceptionHandler` 的通用 Error log 承接，不重複記錄。詳見 [`Docs/ErrorHandlingAndLogging.md`](ErrorHandlingAndLogging.md)（沿用既有 [`Docs/RateLimitingAndAuthLogging.md`](RateLimitingAndAuthLogging.md) 的 auth 事件記錄）
...
- [x] Integration Tests：全域例外處理與操作事件記錄（新專案 `Tests/WebFlow.Tests`，`WebApplicationFactory<Program>` + SQLite，9 個測試：API/HTML 未預期例外的安全回應與 trace id、Timer/CSV 非預期失敗的安全 log metadata、已知並發結果不誤判為 Error、既有 400/401/404/429 業務錯誤維持不變），詳見 [`Docs/ErrorHandlingAndLogging.md`](ErrorHandlingAndLogging.md)。四個 C# 測試專案（`AuthFlow.Tests` 12、`TimerFlow.Tests` 10、`TimeEntryFlow.Tests` 105、`WebFlow.Tests` 9）合計 136 個測試全過
